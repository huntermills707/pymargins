...
def test_strict_mode_rejects_defaults(fit_ols):
    ...
    with pytest.raises(
        ValueError, match="strict=True: argument 'phi' must be explicitly given"
    ):
...
def test_strict_mode_rejects_auto_gradient_backend(fit_ols):
    ...
    with pytest.raises(
        ValueError, match="strict=True: gradient_backend='auto' is not allowed"
    ):
        Margins(
            fit_ols,
            strict=True,
...
def test_strict_mode_rejects_next_missing_arg(fit_ols):
    ...
    with pytest.raises(
        ValueError, match="strict=True: argument 'phi_inv' must be explicitly given"
    ):
...
def test_strict_mode_convenience_constructor_fails_cleanly(fit_ols):
    ...
    with pytest.raises(
        ValueError, match="strict=True: argument 'vcov' must be explicitly given"
    ):
