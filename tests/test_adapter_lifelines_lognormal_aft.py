"""Tests for LifelinesLogNormalAFTAdapter.
"""
...
@pytest.fixture
def lognormal_fit(df_survival):
    ln = LogNormalAFTFitter()
    ln.fit(df_survival, duration_col="T", event_col="E")
    return ln
...
def test_auto_detect_array_requires_training_data(lognormal_fit):
    with pytest.raises(ValueError, match="training_data"):
        auto_detect_adapter(lognormal_fit)
...
def test_predict_matches_lifelines(lognormal_fit, df_survival):
    ...
    sm_pred = lognormal_fit.predict_survival_function(
        df[["x1", "x2"]].iloc[:10], times=[t]
    ).values.flatten()
    np.testing.assert_allclose(our_pred, sm_pred, atol=1e-6)


def test_predict_jax_differentiable(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    df = adapter.training_data
    X = adapter.design_matrix_from_df(df[:10])
...
def test_coefficients_shape(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    # 3 mu params (x1, x2, Intercept) + 1 sigma param (Intercept)
    assert adapter.coefficients().shape == (4,)


def test_covariance_default(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    cov = adapter.covariance()
    assert cov.shape == (4, 4)


def test_covariance_rejects_hc(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    with pytest.raises(ValueError, match="only supports"):
        adapter.covariance("HC0")


def test_covariance_ndarray_override(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    custom = np.eye(4)
...
def test_design_matrix(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    df = adapter.training_data
...
def test_design_matrix_missing_columns(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
...
def test_variable_metadata(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
...
def test_variable_metadata_is_cached(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
...
def test_bootstrap_end_to_end(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    m = Margins(lognormal_fit, adapter=adapter, at="typical",
...
def test_attach_rejects_unsupported_vcov_string(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    with pytest.raises(ValueError, match="LifelinesLogNormalAFTAdapter only supports vcov=None"):
...
def test_attach_rejects_unsupported_vcov_dict(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    with pytest.raises(ValueError, match="LifelinesLogNormalAFTAdapter only supports vcov=None"):
        Margins(lognormal_fit, adapter=adapter, vcov={"type": "cluster", "groups": df_survival["E"]})
...
def test_refit_array(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
    new_adapter = adapter.refit(df_survival)
    assert isinstance(new_adapter, LifelinesLogNormalAFTAdapter)
...
def test_refit_changes_coefficients(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
...
def test_default_prediction_time(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(lognormal_fit, training_data=df_survival)
...
def test_custom_prediction_time(lognormal_fit, df_survival):
    adapter = LifelinesLogNormalAFTAdapter(
        lognormal_fit, training_data=df_survival, prediction_time=1.0
    )
