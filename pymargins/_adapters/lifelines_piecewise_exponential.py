"""
pymargins._adapters.lifelines_piecewise_exponential

Concrete adapter for lifelines PiecewiseExponentialRegressionFitter.

Predicts survival probability at a fixed time using the fitted baseline
hazard and piecewise exponential structure. Uses WrappedFDAdapter because
the piecewise baseline is nonparametric — no closed-form JAX expression.

Delta-method SEs are INVALID for survival-probability estimands because they
ignore uncertainty in the estimated piecewise baseline hazard. Only bootstrap
...
class LifelinesPiecewiseExponentialAdapter(WrappedFDAdapter):
    """Adapter for lifelines PiecewiseExponentialRegressionFitter.

    Predicts survival probability at a fixed time. Uses finite-difference
    JVP because the piecewise baseline hazard has no closed-form JAX
    expression.

    Parameters
    ----------
    results : fitted lifelines PiecewiseExponentialRegressionFitter
    ...
    def __init__(
        ...
        # Extract covariate names from the first lambda group
        # params_ has MultiIndex [param, covariate]
        params = results.params_
        self._param_groups = params.index.get_level_values(0).unique().tolist()
        first_group = self._param_groups[0]
        self._exog_names = params.loc[first_group].index.tolist()
        self._has_intercept = "Intercept" in self._exog_names

        # Store constructor args needed for refit
        self._breakpoints = getattr(results, "breakpoints", None)
    ...
    def attach(self, session) -> None:
        ...
        validate_vcov_spec(vcov, adapter_name="LifelinesPiecewiseExponentialAdapter")
    ...
    def covariance(self, vcov_spec: Optional[Any] = None) -> jnp.ndarray:
        ...
        raise ValueError(
            f"LifelinesPiecewiseExponentialAdapter only supports vcov=None or ndarray. "
            f"Got {vcov_spec!r}"
        )
    ...
    def refit(self, resampled_data: pd.DataFrame, *, index=None) -> "LifelinesPiecewiseExponentialAdapter":
        ...
        if self._breakpoints is not None:
            ctor_kwargs["breakpoints"] = self._breakpoints

        new_fitter = PiecewiseExponentialRegressionFitter(**ctor_kwargs)
