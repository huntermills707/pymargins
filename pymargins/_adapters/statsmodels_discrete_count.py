...
class StatsmodelsDiscreteCountAdapter(ModelAdapter):
    ...
    def refit(self, resampled_data: pd.DataFrame) -> "StatsmodelsDiscreteCountAdapter":
        formula = getattr(self.results.model, "formula", None)
        if formula is not None:
            model_cls_name = type(self.results.model).__name__
            if model_cls_name == "Poisson":
                from statsmodels.formula.api import poisson as smf_poisson
                new_results = smf_poisson(formula, data=resampled_data).fit(disp=False)
            elif model_cls_name == "NegativeBinomial":
                from statsmodels.formula.api import negativebinomial as smf_nb
                new_results = smf_nb(formula, data=resampled_data).fit(disp=False)
            elif model_cls_name == "NegativeBinomialP":
                from statsmodels.discrete.discrete_model import NegativeBinomialP
                new_results = NegativeBinomialP.from_formula(
        ...
        if "const" in self._exog_names or "Intercept" in self._exog_names:
            ...
            if intercept_name not in resampled_data.columns:
                # statsmodels count models do NOT add a constant automatically,
                # so we must add it explicitly if the original model had one.
                pass
        ...
        kwargs = self._collect_original_fit_kwargs()
        if model_cls_name == "Poisson":
            new_results = sm.Poisson(endog, exog_df, **kwargs).fit(disp=False)
        elif model_cls_name == "NegativeBinomial":
            new_results = sm.NegativeBinomial(endog, exog_df, **kwargs).fit(disp=False)
        elif model_cls_name == "NegativeBinomialP":
            new_results = sm.NegativeBinomialP(endog, exog_df, **kwargs).fit(disp=False)
        elif model_cls_name == "GeneralizedPoisson":
            new_results = sm.GeneralizedPoisson(endog, exog_df, **kwargs).fit(disp=False)
        else:
            raise ValueError(f"Unknown model class: {model_cls_name}")
        return StatsmodelsDiscreteCountAdapter(new_results, training_data=resampled_data)
